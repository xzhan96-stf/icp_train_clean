...
if __name__ == '__main__':
    ...
    data_dir = 'H:\\My Drive\\Paper\\CP Shapley\\COVID'
    result_dir = 'H:\\My Drive\\Paper\\CP Shapley\\CPSC Shapley\\Proper Train 0.8 Calibration 0.2\\COVID outlier 0.1 flip -0.2'
    os.chdir(data_dir)

    ### --- COVID ICU Prediction --- ###
    x = pd.read_csv('COVID_entire_set.csv').values
    y = np.load('COVID_entire_set.npy').astype(int)

    scaler.fit(x)
    x = scaler.transform(x)


    noises = [0.3,0.4,0.5,0.6,0.7,0.8]
    #2. dataset partition
    for noise in noises:
        for repeat in range(30):
            #2. Partition proper train (polluted data)/calibration/validation/test sets
            x_train,x_test_val,y_train,y_test_val = train_test_split(x, y, test_size=0.3, stratify = y, random_state=9001+repeat)
            ...
            Cs = [0.1,1,10] # Hyperparameter for the feature selection process

            results_dict = {'Delta':[],'T':[],'Model': [], 'Raw AUROC Val':[],'Clean AUROC Val': [],'Raw AUROC Test':[],'Clean AUROC Test': [],'Raw AUPRC Val':[],'Clean AUPRC Val': [],'Raw AUPRC Test':[],'Clean AUPRC Test': []}
            ...
            for delta in deltas:
                for temperature in temperatures:
                    for Lasso_C in Cs:
                        ...
                        X_pro_train_smote, Y_pro_train_smote = sampling.fit_resample(x_pro_train, y_pro_train)
                        ...
                        y_pro_train_clean_smote, wrongly_labeled_idx, flipped_label, outlier_idx = cpsc.CPSC_clean(x_pro_train_lasso, Y_pro_train_smote, outlier_threshold = 0.1, wrongly_labeled_threshold = -0.2)
                        ...
                        for idx in range(len(model_names)):
                            ...
                            y_pred_val = mdl.predict_proba(x_val_lasso)
                            raw_AUROC_val = roc_auc_score(y_val, y_pred_val[:, 1])
                            raw_AUPRC_val = AUPRC(y_val, y_pred_val[:, 1])
                            print('Raw training set, validation set AUROC: ',raw_AUROC_val)
                            print('Raw training set, validation set AUPRC: ', raw_AUPRC_val)

                            y_pred = mdl.predict_proba(x_test_lasso)
                            raw_AUROC_test = roc_auc_score(y_test, y_pred[:, 1])
                            raw_AUPRC_test = AUPRC(y_test, y_pred[:, 1])
                            print('Raw training set, test set AUROC: ',raw_AUROC_test)
                            print('Raw training set, test set AUPRC: ', raw_AUPRC_test)
                            ...
                            y_pred_val = mdl.predict_proba(x_val_lasso)
                            clean_AUROC_val = roc_auc_score(y_val, y_pred_val[:, 1])
                            clean_AUPRC_val = AUPRC(y_val, y_pred_val[:, 1])
                            print('Clean training set, validation set AUROC: ', clean_AUROC_val)
                            print('Clean training set, validation set AUPRC: ', clean_AUPRC_val)
                            y_pred = mdl.predict_proba(x_test_lasso)
                            clean_AUROC_test = roc_auc_score(y_test, y_pred[:, 1])
                            clean_AUPRC_test = AUPRC(y_test, y_pred[:, 1])
                            print('Clean training set, test set AUROC: ', clean_AUROC_test)
                            print('Clean training set, test set AUPRC: ', clean_AUPRC_test)

                            results_dict['Delta'].append(delta)
                            results_dict['T'].append(temperature)
                            results_dict['Model'].append(name)
                            results_dict['Raw AUROC Val'].append(raw_AUROC_val)
                            results_dict['Clean AUROC Val'].append(clean_AUROC_val)
                            results_dict['Raw AUROC Test'].append(raw_AUROC_test)
                            results_dict['Clean AUROC Test'].append(clean_AUROC_test)
                            results_dict['Raw AUPRC Val'].append(raw_AUPRC_val)
                            results_dict['Clean AUPRC Val'].append(clean_AUPRC_val)
                            results_dict['Raw AUPRC Test'].append(raw_AUPRC_test)
                            results_dict['Clean AUPRC Test'].append(clean_AUPRC_test)
